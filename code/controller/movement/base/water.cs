...
namespace Momentum
{
	public partial class Water : Accelerate
	{
		...
		public virtual Vector3 CheckWaterJump(Vector3 velocity, Vector3 position, BaseController controller)
		{
		...
		public virtual void Move(BaseController controller, Vector3 strafe_vel = new Vector3())
		{
			strafe_vel = GetSwimVel((float)controller.MoveProp["MaxSpeed"], controller.GetPlayer().KeyDown(InputButton.Jump));
			...
			float new_speed = GetNewSpeed(speed, (float)controller.MoveProp["WaterFriction"], ref velocity);
			float strafe_vel_length = CapWishSpeed(strafe_vel.Length, (float)controller.MoveProp["SwimSpeed"]);
			...
			if (strafe_vel_length >= 0.1f)
			{
				...
				float accel_speed = CapWishSpeed((float)controller.MoveProp["WaterAccelerate"] * strafe_vel_length * Time.Delta, add_speed);
			...
			if (trace.Fraction == 1.0f)
			{
				...
				if ((bool)controller.MoveProp["AllowAutoMovement"])
					start_trace.WithZ(start_trace.z + (float)controller.MoveProp["StepSize"] + 1);
